...
structure ChaseBranch (obs : ObsoletenessCondition) (kb : KnowledgeBase) where 
  ...
  fairness : ∀ trg : (RTrigger obs kb.rules), ∃ i : Nat, ((branch.infinite_list i).some_and (fun fs => ¬ trg.val.active fs.fact)) 
    ∧ (∀ j : Nat, j > i -> (branch.infinite_list j).none_or (fun fs => ¬ trg.val.active fs.fact))

namespace ChaseBranch
  def result (branch : ChaseBranch obs kb) : FactSet := 
    fun f => ∃ n : Nat, (branch.branch.infinite_list n).some_and (fun fs => f ∈ fs.fact)
...
structure ChaseTree (obs : ObsoletenessCondition) (kb : KnowledgeBase) where
  ...
  fairness_infinite_branches : ∀ trg : (RTrigger obs kb.rules), ∃ i : Nat, ∀ node : List Nat, node.length ≥ i -> 
    (tree.get node).none_or (fun fs => ¬ trg.val.active fs.fact)
...
theorem chaseBranchSetIsSubsetOfNext (cb : ChaseBranch obs kb) : ∀ n : Nat, 
  ...
  | .some fs => (cb.branch.infinite_list (n+1)).none_or (fun fs2 => fs.fact ⊆ fs2.fact) := 
by
  intro n
...
theorem chaseBranchSetIsSubsetOfAllFollowing (cb : ChaseBranch obs kb) : ∀ (n m : Nat), 
  ...
  | .some fs => (cb.branch.infinite_list (n+m)).none_or (fun fs2 => fs.fact ⊆ fs2.fact) := 
by
  intro n m
  induction m with
  | zero => 
...
theorem chaseTreeSetIsSubsetOfAllFollowing (ct : ChaseTree obs kb) : ∀ (n m : List Nat), 
  ...
  | .some fs => (ct.tree.get (m ++ n)).none_or (fun fs2 => fs.fact ⊆ fs2.fact) :=
by 
  intro n m
  induction m with
  | nil => 
...
theorem chaseBranchSetIsSubsetOfResult (cb : ChaseBranch obs kb) : ∀ n : Nat, (cb.branch.infinite_list n).none_or (fun fs => fs.fact ⊆ cb.result) := by 
  intro n 
  unfold Option.none_or
  ...
  | some fs => 
    ...
    simp [Set.element]
    exists n
    unfold Option.some_and
    simp [eq]
    exact h

theorem chaseTreeSetIsSubsetOfResult (ct : ChaseTree obs kb) : ∀ node : List Nat, (ct.tree.get node).none_or (fun fs => ∀ branch, branch ∈ ct.branches_through node -> fs.fact ⊆ branch.result) := by 
  intro node
  unfold Option.none_or
  ...
  | some fs =>
    ...
    simp [eq, Option.none_or] at branch_subs_result
    apply branch_subs_result

theorem trgLoadedForChaseResultMeansLoadedAtSomeIndex (cb : ChaseBranch obs kb) : ∀ trg : Trigger obs, trg.loaded cb.result -> ∃ n : Nat, (cb.branch.infinite_list n).some_and (fun fs => trg.loaded fs.fact) := by
  intro trg
  unfold ChaseBranch.result
    ...
    cases ex_head_n with | intro i hi =>
      cases ex_tail_n with | intro j hj => 
        ...
        case h_1 heq => 
          ...
          split at heq
          . rw [heq] at hj
            simp [Option.some_and] at hj
          . rw [heq] at hi
            simp [Option.some_and] at hi

        case h_2 heq => 
          simp
          rw [Set.unionSubsetEachSubset]
          constructor 
          . unfold Option.some_and at hi
            split at hi
            . contradiction
            case h_2 a heq2 => 
              ...
              . cases Nat.le.dest help_i with | intro m hm => 
                ...
                simp [Option.none_or] at subsOfAllFollowing
...
theorem trgActiveForChaseResultMeansActiveAtSomeIndex (cb : ChaseBranch obs kb) : ∀ trg : Trigger obs, trg.active cb.result -> ∃ n : Nat, (cb.branch.infinite_list n).some_and (fun fs => trg.active fs.fact) := by 
  intro trg
  ...
  cases trgLoadedForN with | intro n loadedN => 
    ...
    unfold Option.some_and
    split
    case h_1 heq => rw [heq] at loadedN; simp [Option.some_and] at loadedN
    case h_2 heq =>
      simp
      constructor 
      . rw [heq] at loadedN; simp [Option.some_and] at loadedN; exact loadedN
      . intro contra 
        apply active
        apply obs.monotone
        . have subsetOfResult := chaseBranchSetIsSubsetOfResult cb n
          rw [heq] at subsetOfResult
          simp [Option.none_or] at subsetOfResult
    ...
    cases (trgActiveForChaseResultMeansActiveAtSomeIndex cb trg ⟨trg_loaded, trg_not_obsolete⟩) with | intro i active_i => 
      ...
      cases not_active_eventually with | intro j not_active => 
        ...
        case h_2 heq => 
          ...
          simp [Option.none_or] at step_j_subs_result
