...
def exists_trigger_list (obs : ObsoletenessCondition) (rules : RuleSet) (before : ChaseNode obs rules) (after : List (ChaseNode obs rules)) : Prop :=
  ∃ trg : (RTrigger obs rules), trg.val.active before.fact ∧ trg.val.result.enum_with_lt.map (fun ⟨i, fs⟩ => {
    fact := before.fact ∪ fs,
    origin := some ⟨trg, i⟩,
      ...
      | inr f_not_in_j =>
        ...
        | inl trg_ex =>
          ...
          cases trg_ex with | intro trg' htrg' =>
            have head_lt_tail_children_length : head < (ct.tree.children tail).length := by sorry
        ...
        cases fact_in_chase with | intro fact fact_in_chase =>
          ...
          | inr fact_in_trg_result =>
            ...
            | var v_from_head_atom =>
              have : trg.val.rule.head.enum[head_index_for_m_subs.val].fst = head_index_for_m_subs.val := by sorry
              rw [this]
