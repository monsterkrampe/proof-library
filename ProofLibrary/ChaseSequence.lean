...
def exists_trigger_list_condition (obs : ObsoletenessCondition) (rules : RuleSet) (before : ChaseNode obs rules) (after : List (ChaseNode obs rules)) (trg : RTrigger obs rules) : Prop :=
  trg.val.active before.fact ∧ trg.val.result.enum_with_lt.map (fun ⟨i, fs⟩ => {
    fact := before.fact ∪ fs,
    origin := some ⟨trg, i⟩,
    fact_contains_origin_result := by
      have : fs = trg.val.result[i.val] := by sorry
            ...
            have : ∃ f, f ∈ trg'.val.result[head] ∧ (trg.val.apply_to_var_or_const result_index (VarOrConst.var var)) ∈ List.toSet f.terms := by
              cases exis_f with | intro f hf =>
                ...
                . have f_in_next_step := hf.left
                  ...
                  rw [List.enum_with_lt_getElem_snd_eq_getElem] at f_in_next_step
                  cases f_in_next_step with
                  | inr _ => assumption
                  | inl f_in_j =>
                    apply False.elim
...
theorem inductive_homomorphism_latest_index_lt_trg_result_length
  ...
  /- (trg_cond : exists_trigger_list_condition obs kb.rules prev_node (ct.tree.children (inductive_homomorphism ct m m_is_model prev_step).val.1) trg) -/
  ...
  : (inductive_homomorphism ct m m_is_model (prev_step+1)).val.1.head (by apply inductive_homomorphism_path_not_empty) < (Classical.choose trg_ex).val.result.length := by
    ...
    have : inductive_homomorphism_with_prev_node ct m m_is_model prev_step (inductive_homomorphism ct m m_is_model prev_step) prev_node (by rw [prev_ex]) = inductive_homomorphism_with_prev_node_and_trg ct m m_is_model prev_step (inductive_homomorphism ct m m_is_model prev_step) prev_node (by rw [prev_ex]) trg_ex := by
  ...
  | some node =>
    ...
    | succ j ih =>
      ...
      | some node2 =>
        ...
        apply next_step
        . sorry -- involve chaseTreeSetIsSubsetOfAllFollowing
        . exact precond.right
