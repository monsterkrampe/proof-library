...
namespace List
  ...
  -- FIXME: enum_with_lt seems to be totally unusable in proofs; define it via enum instead I suppose
  ...
  /- theorem enum_with_lt_length_eq_of_totalLength_eq (l : List α) (start totalLength totalLength' : Nat) (h : start + l.length = totalLength) (h' : totalLength = totalLength') : (l.enum_with_lt_from start totalLength h).length = (l.enum_with_lt_from start totalLength' (by rw [← h']; exact h)).length := by -/
  /-   induction l generalizing start with -/
  /-   | nil => simp [enum_with_lt_from] -/
  /-   | cons head tail ih => -/
  /-     simp [enum_with_lt_from] -/
  /-     apply ih -/

  /- theorem enum_with_lt_from_length_eq (l : List α) : ∀ n, (l.enum_with_lt_from n (n + l.length) rfl).length = l.length := by -/
  /-   induction l with -/
  /-   | nil => intros; simp [enum_with_lt_from] -/
  /-   | cons head tail ih => -/
  /-     intro n -/
  /-     simp [enum_with_lt_from] -/
  /-     specialize ih (n+1) -/
  /-     simp [← ih] -/
  /-     apply enum_with_lt_length_eq_of_totalLength_eq -/
  /-     rw [Nat.add_assoc, Nat.add_comm _ 1] -/

  /- theorem enum_with_lt_from_getElem_snd_eq_getElem (l : List α) (index : Fin l.length) : ∀ n, ((l.enum_with_lt_from n (n + l.length) rfl)[index]'(by rw [enum_with_lt_from_length_eq]; exact index.isLt)).snd = l[index] := by -/
  /-   induction l with -/
  /-   | nil => have isLt := index.isLt; simp at isLt -/
  /-   | cons head tail ih => -/
  /-     simp [enum_with_lt_from] -/
  /-     intro n -/
