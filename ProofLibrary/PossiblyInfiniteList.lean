...
namespace InfiniteList
  ...
  theorem skip_m_get_sub_eq_get (l : InfiniteList α) (n : Nat) (m : Fin n) : (l.skip m) (n - m) = l n := by 
    unfold skip
    simp
  ...
  theorem combine_skip_take (l : InfiniteList α) (n : Nat) (m : Fin n) : l.take m ++ (l.skip m).take (n-m) = l.take n := by 
    ...
    | succ k ih =>
      ...
      rw [this]
      simp only [take]
      rw [← Nat.sub_add_comm (by apply Nat.le_of_lt_succ; exact m.isLt)]
      rw [skip_m_get_sub_eq_get]
