...
structure ObsoletenessCondition where 
  ...
  contains_trg_result_implies_cond : ∀ {trg} {F} (disj_index : Fin trg.result.length), (trg.result.get disj_index) ⊆ F -> cond trg F
...
def SkolemObsoleteness : ObsoletenessCondition := {
  ...
  contains_trg_result_implies_cond := by
    ...
    rw [List.get_map] at result_in_F
